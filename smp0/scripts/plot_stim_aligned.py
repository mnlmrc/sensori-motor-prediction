...
from smp0.utils import bin_traces, av_across_participants
...
if __name__ == "__main__":
    ...
    av, sd, sem, _ = av_across_participants(channels[datatype], Y)
    ...
    n_channels = len(channels[datatype])
    n_labels = len(cues)

    fig, axs = plt.subplots(n_channels, n_stimF, figsize=(6.4, 8), sharex=True, sharey=True)

    # Plotting data
    for ch, channel in enumerate(channels[datatype]):
        ...
        for sF, stimF in enumerate(stimFingers):
            for l, lab in enumerate(cues[sF]):
                ...
                axs[ch, sF].fill_between(timeAx, avr[sF, l] - semr[sF, l], avr[sF, l] + semr[sF, l],
                                         color=colors[sF][l], lw=0, alpha=.3)
